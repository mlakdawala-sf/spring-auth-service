...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public CustomUserDetailsService(UserRepository userRepository, UserTenantRepository userTenantRepository, UserCredentialRepository userCredentialRepository, RoleRepository roleRepository) {
    ...
    public UserDetails loadUserByUsername(String usernameOrEmail) throws UsernameNotFoundException {
        ...
        Optional<UserCredential> userCredential=userCredentialRepository.findByUserId(user.getId());

        Role role= roleRepository.findById(userTenant.getRoleId()).orElseThrow(() ->
                new UsernameNotFoundException("Role not found by role id: "+ userTenant.getRoleId()));
        GrantedAuthority grantedAuthority = new SimpleGrantedAuthority(role.getName());
        List<GrantedAuthority> listAuthorities = new ArrayList<GrantedAuthority>();
        listAuthorities.add(grantedAuthority);
        return new org.springframework.security.core.userdetails.User(user.getEmail(),
                userCredential.get().getPassword(),listAuthorities);
    }
}
