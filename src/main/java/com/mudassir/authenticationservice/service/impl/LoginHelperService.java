...
//        }
        UserTenant userTenant = userTenantRepository.findUserTenantByUserId(currentUser.getId());
        UserStatus userStatus=userTenant.getStatus();


        if (currentUser.getAuthClientIds().size()==0) {
//            this.logger.error('No allowed auth clients found for this user in DB');

            throw new HttpServerErrorException(HttpStatus.UNPROCESSABLE_ENTITY,AuthErrorKeys.ClientUserMissing.label);
        } else if (!StringUtils.hasLength(req.getClient_secret())) {
//            this.logger.error('client secret key missing from request object');
            throw new HttpServerErrorException(HttpStatus.BAD_REQUEST,AuthErrorKeys.ClientSecretMissing.label);
            // sonarignore:start
